              ...
              <div class="w-full max-w-full px-3 mx-auto md:flex-0 shrink-0 md:w-6/12 lg:w-4/12">
                <div class="relative flex flex-col p-6 bg-white dark:bg-slate-850 text-slate-700 dark:text-slate-200 border-0 shadow-xl rounded-2xl bg-clip-border">
                  <div class="mb-4 text-center">
                    ...
                    <p class="mb-0 text-sm">Gunakan akun admin yang disetujui</p>
                  </div>

                  @if (session('status'))
                    <div class="px-3 py-2 mb-4 text-sm text-white bg-sky-500 rounded-md">{{ session('status') }}</div>
                  @endif
                  @if ($errors->any())
                  ...
                  <form role="form" method="POST" action="{{ url('/login') }}" class="text-left">
                    @csrf
                    <label class="ml-1 text-sm font-bold text-slate-700">Username</label>
                    <div class="mb-4">
                      <input id="username" name="username" type="text" value="{{ old('username') }}" required autofocus class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white/80 bg-clip-padding py-2 px-3 font-normal text-slate-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" />
                    </div>
                    ...
                    <div class="min-h-6 pl-7 mb-0.5 block">
                      <input id="remember" name="remember" class="rounded-1.4 checked:bg-gradient-to-tl checked:from-blue-500 checked:to-violet-500 after:rounded-1.4 after:shadow-checked after:content[''] peer relative float-left -ml-7 mt-0.5 h-5 w-5 cursor-pointer appearance-none border border-gray-200 bg-white bg-contain bg-center p-0 transition-all checked:border-0" type="checkbox" />
                      <label class="inline-block mb-2 ml-1 text-sm font-normal cursor-pointer select-none text-slate-700" for="remember">Ingat saya</label>
                    </div>
                  ...
                  <p class="mt-4 mb-0 text-sm text-center">
                    Belum punya akun?
                    <a href="{{ route('register') }}" class="font-bold text-slate-700">Daftar</a>
                  </p>
                </div>
              </div>
