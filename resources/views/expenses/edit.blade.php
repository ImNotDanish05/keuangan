  ...
  <div class="w-full px-3 mx-auto">
    <div class="relative flex flex-col min-w-0 bg-white dark:bg-slate-850 shadow-xl rounded-2xl text-white">
      ...
      <div class="flex-auto p-6">
        <form method="POST" action="{{ route('expenses.update', $expense) }}" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          @csrf @method('PUT')
          <div>
          ...
          <div>
            ...
            <input type="date" name="spent_at" value="{{ old('spent_at', $expense->spent_at->format('Y-m-d')) }}" required class="border border-gray-300 rounded-lg py-2 px-3 w-full" />
          </div>
          <div>
            <label class="block mb-1 text-xs font-bold text-white">Nominal (Rp)</label>
            <input type="number" name="amount" value="{{ old('amount', $expense->amount) }}" min="0" step="0.01" required class="border border-gray-300 rounded-lg py-2 px-3 w-full" />
          </div>
          <div class="md:col-span-2">
            <label class="block mb-1 text-xs font-bold text-white">Deskripsi</label>
            <textarea name="description" rows="3" class="border border-gray-300 rounded-lg py-2 px-3 w-full">{{ old('description', $expense->description) }}</textarea>
          </div>
