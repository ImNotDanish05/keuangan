...
@section('content')
  <div class="w-full px-3 mx-auto">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-xl font-bold text-white">Pengeluaran</h1>
      <a class="px-4 py-2 text-sm font-bold text-white bg-rose-500 rounded-lg" href="{{ route('expenses.create') }}">+ Tambah</a>
    </div>

    <div class="relative flex flex-col min-w-0 mb-6 bg-white dark:bg-slate-850 shadow-xl rounded-2xl text-white">
      <div class="p-6">
        <form class="flex flex-wrap items-end gap-3" method="get" action="{{ route('expenses.index') }}">
          <div>
        ...
        <table class="items-center w-full mb-0 align-top border-collapse text-white">
          ...
          <tbody>
            @forelse ($expenses as $e)
              <tr>
                <td class="p-2 align-middle bg-transparent whitespace-nowrap"><span class="ml-2 text-sm font-semibold text-white">{{ $e->spent_at->format('Y-m-d') }}</span></td>
                <td class="p-2 align-middle bg-transparent whitespace-nowrap"><span class="ml-2 text-sm font-semibold text-white">{{ $e->category?->name }}</span></td>
                <td class="p-2 text-right align-middle bg-transparent whitespace-nowrap"><span class="ml-2 text-sm font-bold text-rose-600">Rp {{ number_format($e->amount, 0, ',', '.') }}</span></td>
                <td class="p-2 align-middle bg-transparent"><span class="ml-2 text-sm">{{ $e->description }}</span></td>
                <td class="p-2 text-right align-middle bg-transparent whitespace-nowrap">
                  <a class="px-3 py-1 mr-1 text-xs font-bold text-white bg-slate-700 rounded-lg" href="{{ route('expenses.edit', $e) }}">Edit</a>
                  <form action="{{ route('expenses.destroy', $e) }}" method="POST" class="inline" onsubmit="return confirm('Hapus item ini?')">
                    @csrf @method('DELETE')
