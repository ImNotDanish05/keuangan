  ...
  <div class="w-full px-3 mx-auto">
    <div class="relative flex flex-col min-w-0 bg-white dark:bg-slate-850 shadow-xl rounded-2xl text-white">
      ...
      <div class="flex-auto p-6">
        <form method="POST" action="{{ route('incomes.store') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          @csrf
          <div>
          ...
          <div>
            ...
            <input type="date" name="received_at" value="{{ old('received_at', date('Y-m-d')) }}" required class="border border-gray-300 rounded-lg py-2 px-3 w-full" />
          </div>
          <div>
